...
public:
	inline void initial(ModelParams& model, HyperParams& opts, AlignedMemoryPool* mem = NULL){
		...
		_word_avg_pooling.init(opts.hiddenSize, -1, mem);
		_word_max_pooling.init(opts.hiddenSize, -1, mem);
		_word_min_pooling.init(opts.hiddenSize, -1, mem);
		...
		_concat_seg_att.init(opts.hiddenSize, opts.dropProb, mem);
...
public:
	...
	inline void forward(const Instance& inst, bool bTrain = false){
		...
		_word_avg_pooling.forward(this, getPNodes(_hidden, word_num));
		_word_max_pooling.forward(this, getPNodes(_hidden, word_num));
		_word_min_pooling.forward(this, getPNodes(_hidden, word_num));
		_word_pooling_concat.forward(this, &_word_avg_pooling, &_word_max_pooling, &_word_min_pooling);
		...
		_concat_seg_att.forward(this, &_word_pooling_concat, &_att_pooling_concat);
